...
class ScreenManager {
    ...
    spawnFloatCards(bgId, count) {
        ...
        picks.forEach((im, i) => {
            ...
            el.innerHTML = `<img src="${im.realArt}" alt="${im.name}" onerror="this.style.display='none'">`;
            bg.appendChild(el);
...
class TooltipManager {
    ...
    _displayCard(card, e) {
        ...
        if (card.realArt) {
            img.src = card.realArt; img.classList.add('has-img');
    ...
    _displayRift(card, e) {
        ...
        const img = document.getElementById('ttImg');
        const artEl = document.getElementById('ttArt');
        if (card.realArt) { img.src = card.realArt; img.classList.add('has-img'); img.onerror = () => { img.src = ''; img.classList.remove('has-img'); artEl.style.display = ''; }; img.style.borderBottomColor = rarColor + '55'; artEl.style.display = 'none'; }
    ...
    _displayImmortal(im, e) {
        ...
        const artEl = document.getElementById('ttArt');
        if (im.realArt) {
            img.src = im.realArt; img.classList.add('has-img');
            img.onerror = () => { img.src = ''; img.classList.remove('has-img'); artEl.style.display = ''; };
...
class UIRenderer {
    ...
    riftCardHtml(card, extraCls = '') {
        ...
        const img = card.realArt
            ? `<img src="${card.realArt}" alt="${card.name}" style="width:100%;height:100%;object-fit:cover;display:block;" onerror="this.style.display='none'">`
    ...
    immortalPickHtml(im) {
        return `<img src="${im.realArt}" alt="${im.name}" style="width:100%;height:100%;object-fit:contain;display:block;border-radius:8px;">`;
    }

    /** Compact immortal art for in-game sidebar */
    immortalSidebarHtml(im) {
        if (!im.realArt) return null;
        return `<img src="${im.realArt}" alt="${im.name}" style="width:100%;height:auto;object-fit:contain;display:block;border-radius:6px;cursor:pointer;" title="Tap to activate: ${im.abilityName}">`;
    }
...
class Game {
    ...
    async startGameNow() {
        ...
        if (gs.aiImmortal) {
            if (gs.aiImmortal.realArt) { oppImEl.innerHTML = '<img src="' + gs.aiImmortal.realArt + '" style="width:100%;height:100%;object-fit:cover;display:block;border-radius:4px;">'; oppImEl.style.cssText = 'width:70px;height:98px;overflow:hidden;border-radius:4px;'; }
    ...
    render() {
        ...
        if (this.tab === 'immortal') {
            ...
            imms.forEach(im => {
                ...
                if (im.realArt) { card.style.cssText = 'padding:0;overflow:hidden;cursor:default;'; card.innerHTML = `<img src="${im.realArt}" style="width:100%;height:100%;object-fit:cover;display:block;">`; }
...
class DeckBuilderUI {
    ...
    renderImmortals() {
        ...
        list.forEach(im => {
            ...
            if (im.realArt) { card.style.cssText = 'padding:0;overflow:hidden;'; card.innerHTML = `<img src="${im.realArt}" style="width:100%;height:100%;object-fit:cover;display:block;">`; }
    ...
    renderImmortalPreview() {
        ...
        if (this.immortal.realArt) el.innerHTML = `<img src="${this.immortal.realArt}" style="width:100%;border-radius:6px;display:block;margin-bottom:8px;">`;
