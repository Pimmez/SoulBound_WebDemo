...
class CardLoader {
    ...
    static FALLBACK = {
        ...
        cardback:  {"name":"Soul Bound Standard","image":"art/cardback.png","fallbackGradient":"linear-gradient(160deg, #1a1428 0%, #0d0a14 50%, #1a1428 100%)","fallbackIcon":"⚔","borderColor":"#3a3050"},
    };
...
class UIRenderer {
    ...
    /** Render card back image (used for deck blocks, opp hand, draw animations) */
    static cardBackHtml(width, height) {
        const cb = CARDBACK || {};
        if (cb.image) {
            return `<img src="${UIRenderer.encodeArtPath(cb.image)}" style="width:${width};height:${height};object-fit:cover;display:block;border-radius:4px;" onerror="this.onerror=null;this.style.display='none';this.nextSibling.style.display='flex'"><div style="display:none;width:${width};height:${height};align-items:center;justify-content:center;font-size:1.2rem;background:${cb.fallbackGradient || '#1a1428'};border-radius:4px;color:var(--iron3)">${cb.fallbackIcon || '⚔'}</div>`;
        }
        return `<div style="width:${width};height:${height};display:flex;align-items:center;justify-content:center;font-size:1.2rem;background:${cb.fallbackGradient || 'linear-gradient(160deg,#1a1428,#0d0a14,#1a1428)'};border-radius:4px;color:var(--iron3)">${cb.fallbackIcon || '⚔'}</div>`;
    }
    ...
    renderHand() {
        ...
        gs.hand[2].forEach(() => {
            const cbImg = CARDBACK.image ? `<img src="${UIRenderer.encodeArtPath(CARDBACK.image)}" style="width:100%;height:100%;object-fit:cover;display:block;" onerror="this.style.display='none';this.nextSibling.style.display='flex'"><div style="display:none;width:100%;height:100%;align-items:center;justify-content:center;font-size:.42rem;color:var(--iron3)">${CARDBACK.fallbackIcon || '⚔'}</div>` : `<span style="font-size:.42rem;color:var(--iron3)">${CARDBACK.fallbackIcon || '⚔'}</span>`;
            o.innerHTML += `<div class="card-back-mini">${cbImg}</div>`;
        });
    }
...
class Game {
    ...
    async startGameNow() {
        ...
        if (gs.immortal.realArt) {
            ...
            imEl.style.cssText = 'width:100%;height:100%;background:none;border:none;padding:0;cursor:pointer;transition:all .3s;border-radius:10px;';
        ...
        if (gs.aiImmortal) {
            if (gs.aiImmortal.realArt) { oppImEl.innerHTML = '<img src="' + UIRenderer.encodeArtPath(gs.aiImmortal.realArt) + '" style="width:100%;height:100%;object-fit:cover;display:block;border-radius:50%;">'; oppImEl.style.cssText = 'width:100%;height:100%;overflow:hidden;border-radius:50%;'; }
...
class Preloader {
    ...
    _getArtPaths() {
        ...
        if (CARDBACK.image) paths.push(UIRenderer.encodeArtPath(CARDBACK.image));
